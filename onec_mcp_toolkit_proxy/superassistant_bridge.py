                ...
                return {
                    ...
                    "result": {
                        ...
                        "serverInfo": {
                            ...
                            "version": "1.0.0",
                            "instructions": """Вы работаете с базой данных 1С:Предприятие через MCP инструменты. 

ВАЖНЫЕ ПРАВИЛА:
1. ВСЕГДА используйте get_metadata для изучения структуры базы перед выполнением запросов
2. В запросах используйте параметры через &ИмяПараметра (например: ГДЕ Код = &КодТовара)
3. ОБЯЗАТЕЛЬНО добавляйте условие 'ГДЕ НЕ ПометкаУдаления' для справочников и документов
4. В execute_code НЕ ставьте точку с запятой в конце выражений
5. Используйте 'Результат = ...' для возврата значений из execute_code

ПОСЛЕДОВАТЕЛЬНОСТЬ РАБОТЫ:
1. Сначала get_metadata для понимания структуры
2. Затем execute_query с правильными именами полей и параметрами
3. При необходимости get_object_by_link для детальной информации
4. find_references_to_object для поиска связанных данных

ПРИМЕРЫ:
- get_metadata с filter="Справочник.Номенклатура" для изучения структуры
- execute_query: "ВЫБРАТЬ Код, Наименование ИЗ Справочник.Номенклатура ГДЕ НЕ ПометкаУдаления И Код = &КодТовара"
- execute_code: "Результат = ТекущаяДата()" (без точки с запятой!)"""
                        }
                    }
                }
                ...
                return {
                    ...
                    "result": {
                        "tools": [
                            ...
                            {
                                "name": "execute_code",
                                "description": "Выполнить код 1С. Используйте 'Результат = ...' для возврата значений. НЕ объявляйте процедуры/функции. НЕ ставьте точку с запятой в конце выражений.",
                                "inputSchema": {
                                    "type": "object",
                                    "properties": {
                                        "code": {
                                            "type": "string",
                                            "description": "Код 1С для выполнения. Используйте 'Результат = ...' для возврата значения. НЕ объявляйте процедуры/функции, НЕ используйте 'Возврат', НЕ ставьте точку с запятой в конце. Пример: 'Результат = ТекущаяДата()' (без точки с запятой)"
                                        }
                                    },
                                    "required": ["code"]
                                }
                            },
                            {
                                ...
                                "inputSchema": {
                                    ...
                                    "properties": {
                                        ...
                                        "extension_name": {
                                            "type": "string",
                                            "description": "Имя расширения конфигурации. Значения: None или пустая строка для основной конфигурации, строка с именем расширения для объектов расширения. В обычной форме не поддерживается."
                                        } строка = основная конфигурация, '' = список всех расширений, 'ИмяРасширения' = объекты конкретного расширения"
                                        }
                                    }
                                }
                            },
