...
Функция ВыполнитьКомандуНаСервере(Команда)
	...
	ИначеЕсли ИмяИнструмента = "get_metadata" Тогда
		...
		Возврат ПолучитьМетаданные(Фильтр, ТипМетаданных, МаскаИмени, Лимит, Смещение, Секции);
	...
	ИначеЕсли ИмяИнструмента = "get_event_log" Тогда
		...
		Возврат ПолучитьЖурналРегистрации(ДатаНачала, ДатаОкончания, Уровни, Лимит);
	...
	ИначеЕсли ИмяИнструмента = "find_references_to_object" Тогда
		...
		Возврат НайтиСсылкиНаОбъект(ОписаниеЦелевогоОбъекта, ОбластиПоиска, ФильтрМетаданных, ЛимитНайденных);
	...
	Попытка
		...
		Если ВключитьСхему Тогда
			...
			Для Каждого Колонка Из РезультатЗапроса.Колонки Цикл
				СхемаКолонок.Добавить(Новый Структура("name, type", Колонка.Имя, Строка(Колонка.ТипЗначения)));
			КонецЦикла;
			Схема = СхемаКолонок;
		КонецЕсли;
	...
	Попытка
		...
		Если Прав(КодДляПроверки, 1) = ";" Тогда
			Возврат Новый Структура("success, error", Ложь, 
				"Синтаксическая ошибка: лишняя точка с запятой в конце выражения. Уберите ';' в конце. / Syntax error: extra semicolon at the end of expression. Remove ';' at the end.");
		КонецЕсли;
		
		// Проверяем на использование переменной Результат без объявления
		Если Найти(ВРег(КодДляПроверки), "РЕЗУЛЬТАТ") > 0 И Найти(ВРег(КодДляПроверки), "ПЕРЕМ") = 0 Тогда
			// Автоматически добавляем объявление переменной
			Код = "Перем Результат; " + Код;
			ДобавитьВЛог("Автоматически добавлено объявление переменной 'Результат'");
		КонецЕсли;
		КоличествоКавычек = 0;
		...
		РезультатВыполнения = Вычислить(Код);
		
		Возврат Новый Структура("success, data", Истина, БезопасноеЗначениеДляПередачи(РезультатВыполнения));
	...
	Исключение
		...
		Если Найти(ТекстОшибки, "Ошибка компиляции") > 0 Тогда
			Если Найти(ТекстОшибки, "Ошибка в выражении") > 0 Тогда
				ТекстОшибки = ТекстОшибки + " | Возможные причины: лишняя точка с запятой, незакрытые скобки/кавычки, неправильный синтаксис 1С / Possible causes: extra semicolon, unclosed brackets/quotes, incorrect 1C syntax";
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьМетаданные(Фильтр, ТипМетаданных, МаскаИмени, Лимит, Смещение, Секции)
...
КонецФункции
Функция ПолучитьЖурналРегистрации(ДатаНачала, ДатаОкончания, Уровни, Лимит)
	
	Попытка
		
		// Формируем отбор для ВыгрузитьЖурналРегистрации
		...
		// Проверяем, что отбор не пустой, иначе передаем Неопределено
		ОтборДляВыгрузки = ?(Отбор.Количество() = 0, Неопределено, Отбор);
		
		Если Лимит <> Неопределено И Лимит > 0 Тогда
			ВыгрузитьЖурналРегистрации(ТаблицаСобытий, ОтборДляВыгрузки, , , Лимит);
		Иначе
			ВыгрузитьЖурналРегистрации(ТаблицаСобытий, ОтборДляВыгрузки);
		КонецЕсли;
		
		МассивДанных = Новый Массив;
		Для Каждого Событие Из ТаблицаСобытий Цикл
			СтрокаСобытия = Новый Структура;
			СтрокаСобытия.Вставить("Дата", Событие.Дата);
			СтрокаСобытия.Вставить("Пользователь", Строка(Событие.Пользователь));
			СтрокаСобытия.Вставить("Событие", Строка(Событие.Событие));
			СтрокаСобытия.Вставить("Уровень", Строка(Событие.Уровень));
			СтрокаСобытия.Вставить("Комментарий", Строка(Событие.Комментарий));
			СтрокаСобытия.Вставить("Метаданные", Строка(Событие.Метаданные));
			СтрокаСобытия.Вставить("Данные", БезопасноеЗначениеДляПередачи(Событие.Данные));
			МассивДанных.Добавить(СтрокаСобытия);
		КонецЦикла;
		
		Возврат Новый Структура("success, data", Истина, МассивДанных);
...
Функция НайтиСсылкиНаОбъект(ОписаниеЦелевогоОбъекта, ОбластиПоиска, ФильтрМетаданных, ЛимитНайденных)
	...
	Попытка
		...
		БюджетМиллисекунд = 30 * 1000; // 30 секунд по умолчанию
		ЛимитНаМетуОбъект = 10; // Лимит на один объект метаданных
...
Функция ПолучитьПраваДоступа(ИмяОбъектаМетаданных, ИмяПользователяИБ, ФильтрПрав, ФильтрРолей)
	...
	Попытка
		...
		Если НЕ ПустаяСтрока(ИмяОбъектаМетаданных) Тогда
			// Здесь должна быть реализация получения прав для конкретного объекта
			МассивПрав.Добавить(Новый Структура("Объект, Право, Значение", ИмяОбъектаМетаданных, "Чтение", Истина));
		КонецЕсли;
